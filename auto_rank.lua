...
-- Кфг: getgenv().AutoRank = { … } поверх встроенных дефолтов; опционально readfile("auto_rank_defaults.lua") подменяет таблицу целиком.
local LocalPlayer = Players.LocalPlayer
local autoRankLoadTick = tick()
-- Скрипт-версия (должна быть объявлена до AR.Log.resetFile).
local AUTO_RANK_RUNTIME_VERSION = 7

--[[ NAV: встроенные дефолты + опц. readfile | Net/log | ARQ quest | AR.ARC hatch | Farm | Heartbeat / AR.HB.tasks ]]
...
-- Дефолты: встроенная таблица; при наличии файла — readfile("auto_rank_defaults.lua") перезаписывает (без HttpGet).
local function loadAutoRankInternalDefaults()
	...
	-- При no_goal из-за missing GoalCmds/FFlags или при generators>0 но validCallbacks==0 (executor ломает Callback): иначе яйца почти не крутятся.
	autoHatchProgressWhenGoalModulesMissing = true,
