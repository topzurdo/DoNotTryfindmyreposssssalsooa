...
-- World-specific overrides (встроенный массив AutoRankEmbeddedWorldProfiles), resolved via PlaceFile.
local AutoRankWorld = {
	active = nil,
	_lastRefresh = 0,
}
...
-- Профили миров встроены (один файл; без подгрузки autorank/worlds по HTTP).
local AutoRankEmbeddedWorldProfiles = {
	{
		id = "w4_future",
		priority = 50,
		detect = function(env)
...
local function refreshAutoRankWorldSelection()
	...
	local mods = {}
	for _, mod in ipairs(AutoRankEmbeddedWorldProfiles) do
		if type(mod) == "table" and type(mod.detect) == "function" then
